(function () {
  ...
  function renderMarketingAssets(filter="all"){
    const root=$("creatorMarketingAssets");if(!root)return;
    const assets=creatorMarketingAssets.filter(asset=>filter==="active"?asset.status==="active":true);
    root.innerHTML=assets.length?assets.map(asset=>{const media=asset.thumbnail_url||asset.media_url;const preview=media?(asset.asset_type==="reel"?'<video src="'+escapeHtml(media)+'" muted playsinline></video>':'<img src="'+escapeHtml(media)+'" alt="">'):'<span>'+escapeHtml(String(asset.asset_type||"asset").toUpperCase())+'</span>';return'<article class="creatorMarketingCard"><div class="creatorMarketingPreview">'+preview+'</div><div class="creatorMarketingCopy"><div class="creatorMarketingMeta">'+escapeHtml(asset.asset_type)+' · Tier '+number(asset.minimum_tier)+'+ · '+escapeHtml(asset.city||"All cities")+'</div><h3>'+escapeHtml(asset.title)+'</h3><div>'+escapeHtml(asset.campaign||"No campaign")+' · '+escapeHtml(asset.status)+'</div><div class="creatorMarketingActions">'+(asset.media_url?'<a class="creatorAction" href="'+escapeHtml(asset.media_url)+'" target="_blank" rel="noopener">Preview</a>':'')+'<button class="creatorAction" type="button" data-edit-asset="'+escapeHtml(asset.id)+'">Edit</button><button class="creatorAction primary" type="button" data-toggle-asset="'+escapeHtml(asset.id)+'" data-asset-status="'+(asset.status==="active"?"inactive":"active")+'">'+(asset.status==="active"?"Deactivate":"Publish")+'</button><button class="creatorAction danger" type="button" data-delete-asset="'+escapeHtml(asset.id)+'">Delete</button></div></div></article>'}).join(""):'<div class="creatorApprovedEmpty">Marketing assets will appear here.</div>'
  }
  ...
  document.addEventListener("click",async event=>{
    ...
    if(marketingTab){const tab=marketingTab.dataset.marketingTab;if(tab==="create")fillAssetForm({});else{resetAssetForm();renderMarketingAssets(tab)}return}
