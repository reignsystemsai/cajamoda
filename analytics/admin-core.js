(function () {
  ...
  function renderMarketingAssets(){
    ...
    const source=marketingAssetsForDisplay(),recentCount=source.filter(isNewMarketingAsset).length;setText("marketingNewCount",creatorMarketingAssets.length?(recentCount?recentCount+" this week":"This week"):"Preview content");
    const matching=source.filter(asset=>(creatorMarketingFilter==="all"||(creatorMarketingFilter==="new"?isNewMarketingAsset(asset):asset.asset_type===creatorMarketingFilter))&&(!query||[asset.title,asset.suggested_text,asset.related_product_name].some(value=>String(value||"").toLowerCase().includes(query))));
    ...
    root.innerHTML=assets.length?assets.map((asset,index)=>{const media=asset.thumbnail_url||asset.media_url;const isVideo=!asset.placeholder&&(asset.asset_type==="reel"||asset.asset_type==="story");const video=isVideo&&!asset.thumbnail_url;const preview=media?(video?'<video src="'+escapeHtml(media)+'" muted playsinline preload="metadata"></video>':'<img src="'+escapeHtml(media)+'" alt="'+escapeHtml(asset.title||"Marketing content")+'">'):'<span>'+escapeHtml(marketingTypeLabel(asset))+'</span>';return'<article class="creatorMarketingCard '+(index===0?'featured':'')+'"><button class="creatorMarketingPreview '+((asset.asset_type==="reel"||asset.asset_type==="story")?'is-video':'')+'" type="button" data-preview-asset="'+escapeHtml(asset.id)+'"><span class="creatorMarketingType">'+escapeHtml(marketingTypeLabel(asset))+'</span>'+preview+'</button><div class="creatorMarketingCopy"><div class="creatorMarketingMeta">'+escapeHtml(asset.placeholder?"Preview placeholder":asset.status==="active"?"Published to creators":"Draft")+'</div><h3>'+escapeHtml(asset.title||"Untitled content")+'</h3><div class="creatorMarketingDescription">'+escapeHtml(asset.suggested_text||"Ready for a caption when you are.")+'</div><div class="creatorMarketingActions">'+(asset.media_url?'<button class="creatorAction download" type="button" data-download-asset="'+escapeHtml(asset.id)+'">Download '+(asset.asset_type==="photo"?"photo":"reel")+'</button>':'')+(asset.suggested_text?'<button class="creatorAction" type="button" data-copy-marketing-text="'+encodeURIComponent(asset.suggested_text)+'">Copy caption</button>':'')+(asset.placeholder?'':'<button class="creatorAction" type="button" data-edit-asset="'+escapeHtml(asset.id)+'">Edit</button><button class="creatorAction primary" type="button" data-toggle-asset="'+escapeHtml(asset.id)+'" data-asset-status="'+(asset.status==="active"?"inactive":"active")+'">'+(asset.status==="active"?"Unpublish":"Publish")+'</button><button class="creatorAction danger" type="button" data-delete-asset="'+escapeHtml(asset.id)+'">Delete</button>')+'</div></div></article>'}).join(""):'<div class="creatorApprovedEmpty">No content matches this view.</div>'
  }
