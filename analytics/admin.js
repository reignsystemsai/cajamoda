(function () {
  ...
  function creatorReferralUrl(slug){return "https://www.cajamoda.com/?utm_source=creator&utm_medium=creator&utm_campaign="+encodeURIComponent(slug)}
  ...
  function renderCreatorNetwork(rows){
    ...
    if(sb)sb.innerHTML=selected.length?selected.map(({person:p})=>{const link=creatorReferralUrl(p.slug);return'<tr><td>'+creatorIdentity(p)+'</td><td>'+escapeHtml(p.location)+'</td><td><div class="creatorShareLink"><code>'+escapeHtml(link)+'</code><button class="creatorAction" type="button" data-copy-creator-link="'+escapeHtml(link)+'">Copiar</button></div></td><td><span class="creatorStatus">Nivel '+number(p.tier)+'</span></td><td>'+number(p.commissionRate)+'%</td><td>'+creatorTierActions(p)+'</td></tr>'}).join(""):'<tr><td class="creatorEmpty" colspan="6">Las creadoras aprobadas aparecerán aquí.</td></tr>';
