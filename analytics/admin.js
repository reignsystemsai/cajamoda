(function () {
  ...
  function creatorPerson(a){const f=String(a?.first_name||""),l=String(a?.last_name||""),instagram=String(a?.instagram_username||"").replace(/^@/,""),tiktok=String(a?.tiktok_username||"").replace(/^@/,""),phoneDigits=String(a?.phone||"").replace(/\D/g,""),submittedDate=a?.created_at?new Date(a.created_at):null,validSubmitted=submittedDate&&!Number.isNaN(submittedDate.getTime());return{id:String(a?.id||""),name:(f+" "+l).trim()||"CajaModa Creator",email:String(a?.email||""),location:[a?.city,a?.department].filter(Boolean).join(", ")||"—",instagram,tiktok,phoneDigits,initials:(f.slice(0,1)+l.slice(0,1)).toUpperCase()||"CM",submittedDate:validSubmitted?new Intl.DateTimeFormat("en-US",{dateStyle:"medium",timeZone:"America/Bogota"}).format(submittedDate):"—",submittedTime:validSubmitted?new Intl.DateTimeFormat("en-US",{timeStyle:"short",timeZone:"America/Bogota"}).format(submittedDate)+" COT":"—",status:String(a?.status||"new"),onboardingStatus:String(a?.onboarding_status||""),slug:String(a?.creator_slug||""),tier:Number(a?.tier||0),commissionRate:Number(a?.commission_rate||0),visits:Number(a?.visits||a?.analytics?.visits||0),orders:Number(a?.paid_orders||a?.analytics?.paid_orders||0),sales:Number(a?.sales_total||a?.analytics?.sales_total||0),conversion:Number(a?.conversion_rate||a?.analytics?.conversion_rate||0)}}
  ...
  function renderCreatorNetwork(rows){
    ...
    if(sb)sb.innerHTML=selected.length?selected.map(({person:p})=>{const link=creatorReferralUrl(p.slug);const state=p.onboardingStatus==="active"?"Active":p.onboardingStatus==="invited"?"Invitation sent":"Pending";return'<article class="creatorApprovedCard"><div class="creatorApprovedTop"><div class="creatorApprovedIdentity">'+creatorIdentity(p)+'<span class="creatorApprovedLocation">'+escapeHtml(p.location)+'</span></div><span class="creatorStatus">'+escapeHtml(state)+'</span></div><div class="creatorApprovedLink"><code>'+escapeHtml(link)+'</code><button class="creatorAction" type="button" data-copy-creator-link="'+escapeHtml(link)+'">Copy link</button></div><div class="creatorApprovedBottom"><div class="creatorPerformance"><strong>'+money(p.sales)+'</strong><span>'+number(p.orders)+' paid orders</span><button class="creatorAction primary" type="button" data-creator-sales-id="'+escapeHtml(p.id)+'" data-creator-sales-name="'+escapeHtml(p.name)+'">View Sales</button></div><div class="creatorPlan"><div class="creatorPlanLabel"><span>Commission plan</span><strong>Tier '+number(p.tier)+' · '+number(p.commissionRate)+'%</strong></div><div class="creatorTierControls">'+[1,2,3].map(tier=>'<button class="creatorAction '+(p.tier===tier?'primary':'')+'" type="button" data-creator-id="'+escapeHtml(p.id)+'" data-creator-status="approved" data-creator-tier="'+tier+'">Tier '+tier+'</button>').join("")+'</div></div></div><div class="creatorApprovedFooter">'+creatorAction(p.id,"declined","Deactivate creator","danger")+'</div></article>'}).join(""):'<div class="creatorApprovedEmpty">Approved creators will appear here.</div>';
