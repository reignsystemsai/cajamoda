(function () {
  ...
  let creatorMarketingFilter = "all";
  ...
  function marketingFileName(asset){const extension=asset.asset_type==="reel"||asset.asset_type==="story"?"mp4":"jpg";return String(asset.title||"cajamoda-content").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")+"."+extension}
  function isNewMarketingAsset(asset){const stamp=new Date(asset.created_at||asset.updated_at||0).getTime();return stamp>0&&Date.now()-stamp<=604800000}
  function renderMarketingAssets(){
    ...
    const recentCount=creatorMarketingAssets.filter(isNewMarketingAsset).length;setText("marketingNewCount",recentCount?recentCount+" this week":"This week");
    const matching=creatorMarketingAssets.filter(asset=>(creatorMarketingFilter==="all"||(creatorMarketingFilter==="new"?isNewMarketingAsset(asset):asset.asset_type===creatorMarketingFilter))&&(!query||[asset.title,asset.suggested_text,asset.related_product_name].some(value=>String(value||"").toLowerCase().includes(query))));
    const lead=matching.findIndex(asset=>asset.asset_type==="reel"),assets=lead>0?[matching[lead],...matching.slice(0,lead),...matching.slice(lead+1)]:matching;
    root.innerHTML=assets.length?assets.map((asset,index)=>{const media=asset.thumbnail_url||asset.media_url;const isVideo=asset.asset_type==="reel"||asset.asset_type==="story";const video=isVideo&&!asset.thumbnail_url;const preview=media?(video?'<video src="'+escapeHtml(media)+'" muted playsinline preload="metadata"></video>':'<img src="'+escapeHtml(media)+'" alt="'+escapeHtml(asset.title||"Marketing content")+'">'):'<span>'+escapeHtml(marketingTypeLabel(asset))+'</span>';return'<article class="creatorMarketingCard '+(index===0?'featured':'')+'"><button class="creatorMarketingPreview '+(isVideo?'is-video':'')+'" type="button" data-preview-asset="'+escapeHtml(asset.id)+'"><span class="creatorMarketingType">'+escapeHtml(marketingTypeLabel(asset))+'</span>'+preview+'</button><div class="creatorMarketingCopy"><div class="creatorMarketingMeta">'+escapeHtml(asset.status==="active"?"Published to creators":"Draft")+'</div><h3>'+escapeHtml(asset.title||"Untitled content")+'</h3><div class="creatorMarketingDescription">'+escapeHtml(asset.suggested_text||"Ready for a caption when you are.")+'</div><div class="creatorMarketingActions">'+(asset.media_url?'<button class="creatorAction download" type="button" data-download-asset="'+escapeHtml(asset.id)+'">Download '+(asset.asset_type==="photo"?"photo":"reel")+'</button>':'')+(asset.suggested_text?'<button class="creatorAction" type="button" data-copy-marketing-text="'+encodeURIComponent(asset.suggested_text)+'">Copy caption</button>':'')+'<button class="creatorAction" type="button" data-edit-asset="'+escapeHtml(asset.id)+'">Edit</button><button class="creatorAction primary" type="button" data-toggle-asset="'+escapeHtml(asset.id)+'" data-asset-status="'+(asset.status==="active"?"inactive":"active")+'">'+(asset.status==="active"?"Unpublish":"Publish")+'</button><button class="creatorAction danger" type="button" data-delete-asset="'+escapeHtml(asset.id)+'">Delete</button></div></div></article>'}).join(""):'<div class="creatorApprovedEmpty">No content matches this view. Use Upload content to add a reel or photo.</div>'
  }
  function openMarketingPreview(asset){const modal=$("marketingPreviewModal"),media=$("marketingPreviewMedia");if(!modal||!media||!asset)return;const video=asset.asset_type==="reel"||asset.asset_type==="story";media.innerHTML=asset.media_url?(video?'<video src="'+escapeHtml(asset.media_url)+'" controls autoplay playsinline></video>':'<img src="'+escapeHtml(asset.media_url)+'" alt="'+escapeHtml(asset.title||"Marketing content")+'">'):'<span>No media uploaded</span>';setText("marketingPreviewType",marketingTypeLabel(asset));setText("marketingPreviewTitle",asset.title||"Content preview");setText("marketingPreviewText",asset.suggested_text||"No caption added.");const actions=$("marketingPreviewActions");if(actions)actions.innerHTML=(asset.media_url?'<button class="creatorAction download" type="button" data-download-asset="'+escapeHtml(asset.id)+'">Download '+(asset.asset_type==="photo"?"photo":"reel")+'</button>':'')+(asset.suggested_text?'<button class="creatorAction" type="button" data-copy-marketing-text="'+encodeURIComponent(asset.suggested_text)+'">Copy caption</button>':'');modal.hidden=false}
  ...
  function renderCreatorNetwork(rows){
    ...
    setText("creatorIncomingCount",number(incoming.length));setText("creatorSelectedCount",number(selected.length));setText("creatorRankedCount",number(ranked.length));const incomingAlert=$("creatorIncomingAlert");if(incomingAlert){incomingAlert.hidden=incoming.length===0;incomingAlert.textContent=incoming.length+" NEW"}
    ...
    if(sb)sb.innerHTML=selected.length?selected.map(({person:p})=>{const link=creatorReferralUrl(p.slug);const state=p.onboardingStatus==="active"?"Active":p.onboardingStatus==="invited"?"Invitation sent":"Pending";return'<article class="creatorApprovedCard is-collapsed"><div class="creatorApprovedTop"><div class="creatorApprovedIdentity">'+creatorIdentity(p,true)+'</div><div class="creatorApprovedTopActions"><span class="creatorStatus">'+escapeHtml(state)+'</span><button class="creatorApprovedToggle" type="button" data-toggle-approved-creator aria-expanded="false" aria-label="Show creator details"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 9 6 6 6-6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></button></div></div><div class="creatorApprovedDetails"><div class="creatorApprovedLink"><code>'+escapeHtml(link)+'</code><button class="creatorAction" type="button" data-copy-creator-link="'+escapeHtml(link)+'">Copy link</button></div><div class="creatorApprovedMeta"><span><b>Email</b>'+escapeHtml(p.email)+'</span><span><b>Location</b>'+escapeHtml(p.location)+'</span><span><b>Payment</b>'+escapeHtml(p.payoutMethod?p.payoutMethod.toUpperCase()+" · "+p.payoutDestination:"Not provided")+'</span><span><b>Agreement</b>'+escapeHtml(p.agreementAccepted)+(p.agreementVersion?" · v"+escapeHtml(p.agreementVersion):"")+'</span></div><div class="creatorApprovedBottom"><div class="creatorPerformance"><strong>'+money(p.sales)+'</strong><span>'+number(p.orders)+' paid orders</span><button class="creatorAction primary" type="button" data-creator-sales-id="'+escapeHtml(p.id)+'" data-creator-sales-name="'+escapeHtml(p.name)+'">View Sales</button></div><div class="creatorPlan"><div class="creatorPlanLabel"><span>Commission plan</span><strong>Tier '+number(p.tier)+' · '+number(p.commissionRate)+'%</strong></div><div class="creatorTierControls">'+[1,2,3].map(tier=>'<button class="creatorAction '+(p.tier===tier?'primary':'')+'" type="button" data-creator-id="'+escapeHtml(p.id)+'" data-creator-status="approved" data-creator-tier="'+tier+'">Tier '+tier+'</button>').join("")+'</div></div></div><div class="creatorApprovedFooter">'+(p.onboardingStatus==="active"?'<button class="creatorAction" type="button" data-creator-password-reset="'+escapeHtml(p.email)+'">Send password reset</button>':'')+'<button class="creatorAction primary" type="button" data-creator-mark-paid="'+escapeHtml(p.id)+'">Mark due commissions paid</button>'+creatorAction(p.id,"declined","Deactivate creator","danger")+'</div></div></article>'}).join(""):'<div class="creatorApprovedEmpty">Approved creators will appear here.</div>';
  ...
  document.addEventListener("click",async event=>{
    ...
    if(marketingTab){fillAssetForm({});return}
    const marketingFilter=event.target.closest("[data-marketing-filter]");if(marketingFilter){creatorMarketingFilter=marketingFilter.dataset.marketingFilter||"all";qsa("[data-marketing-filter]").forEach(button=>button.classList.toggle("active",button===marketingFilter));renderMarketingAssets();return}
    const previewAsset=event.target.closest("[data-preview-asset]");if(previewAsset){openMarketingPreview(creatorMarketingAssets.find(asset=>asset.id===previewAsset.dataset.previewAsset));return}
    if(event.target.closest("[data-close-marketing-preview]")||event.target===$("marketingPreviewModal")){closeMarketingPreview();return}
    const downloadAsset=event.target.closest("[data-download-asset]");if(downloadAsset){await downloadMarketingAsset(creatorMarketingAssets.find(asset=>asset.id===downloadAsset.dataset.downloadAsset));return}
    ...
    const toggleAsset=event.target.closest("[data-toggle-asset]");if(toggleAsset){const asset=creatorMarketingAssets.find(item=>item.id===toggleAsset.dataset.toggleAsset);if(!asset)return;toggleAsset.disabled=true;try{await request("/api/store-owner/creator-marketing-assets/"+encodeURIComponent(asset.id),{method:"PATCH",body:{...asset,status:toggleAsset.dataset.assetStatus,assetType:asset.asset_type,minimumTier:asset.minimum_tier,startAt:asset.start_at,endAt:asset.end_at,suggestedText:asset.suggested_text,relatedProductName:asset.related_product_name,relatedProductUrl:asset.related_product_url}});await loadMarketingAssets()}catch(error){window.alert(error.message)}return}
    ...
    if(passwordResetButton){if(!window.confirm("Send a fresh password-reset link to this creator?"))return;const label=passwordResetButton.textContent;passwordResetButton.disabled=true;passwordResetButton.textContent="Sending…";try{await request("/api/creators/password-reset/request",{method:"POST",body:{email:passwordResetButton.dataset.creatorPasswordReset}});passwordResetButton.textContent="Reset sent";setTimeout(()=>{passwordResetButton.textContent=label;passwordResetButton.disabled=false},1800)}catch(error){passwordResetButton.textContent=label;passwordResetButton.disabled=false;window.alert(error?.message||"The password-reset email could not be sent.")}return}
