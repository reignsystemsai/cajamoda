(function () {
  ...
  function creatorIdentity(p){const activeClass=p.orders>0||p.sales>0?' activeSeller':'',avatar=p.profilePhotoUrl?'<span class="creatorApplicantAvatar hasPhoto'+activeClass+'"><img src="'+escapeHtml(p.profilePhotoUrl)+'" alt=""></span>':'<span class="creatorApplicantAvatar'+activeClass+'">'+escapeHtml(p.initials)+'</span>';return'<div class="creatorApplicant">'+avatar+'<div><strong>'+escapeHtml(p.name)+'</strong><span>'+escapeHtml(p.email)+'</span></div></div>'}
