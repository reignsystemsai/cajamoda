...
const checks = [
  ...
  [server.includes('function isKarolNetworkManager(request)') && server.includes('? ["marketing", "network"] : []') && admin.includes('data-network-manager') && admin.includes('permissions?.includes("network")'), "Karolay owner access includes Network Management and Red de Creadoras"],
