...
const extendedRateMigration = readFileSync(new URL("../supabase/migrations/202609200001_extend_creator_commission_rates.sql", import.meta.url), "utf8");
...
const checks = [
  ...
  [adminScript.includes("[1,2,3,4,5].map") && adminScript.includes("'+(tier*10)+'%") && server.includes("[1, 2, 3, 4, 5].includes") && server.includes("[4, 5].includes(assignedTier)") && extendedRateMigration.includes("commission_rate in (10, 20, 30, 40, 50)"), "Network Manager supports persistent 40% and 50% creator rates"],
