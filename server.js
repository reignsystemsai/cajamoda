...
const checkout3SyncLocks = new Map();
...
function checkout3PaymentMetadata(lines, body, delivery) {
  const metadata = {
    source: "cajamoda-checkout-3",
    checkoutToken: safeText(body?.checkoutToken, 100),
    itemCount: String(lines.length),
    deliveryMethod: safeText(delivery.method, 20),
    deliverySummary: safeText(delivery.title, 300),
    deliveryPromise: safeText(body?.delivery?.promise, 40),
    deliveryFeeCop: String(Math.max(0, Math.round(Number(delivery.fee || 0)))),
    deliveryAddress: safeText(delivery.addressLine, 250),
    deliveryCity: safeText(delivery.city, 100),
    deliveryState: safeText(delivery.state, 100),
    deliveryPostalCode: safeText(delivery.postalCode, 40),
  ...
  lines.forEach((line, index) => {
    metadata[`c3item${index}`] = Buffer.from(JSON.stringify({
      p: line.productId,
      v: line.variantId,
      q: line.quantity,
      a: line.priceCop,
      n: safeText(line.name, 100),
      f: line.fulfillmentCode,
      d: line.selectedDeliveryMode
    })).toString("base64url");
  });
  return metadata;
}

function checkout3PaymentLines(paymentIntent) {
  const count = Math.min(25, Math.max(0, Number(paymentIntent?.metadata?.itemCount || 0)));
  const lines = [];
  for (let index = 0; index < count; index += 1) {
    try {
      const encoded = safeText(paymentIntent?.metadata?.[`c3item${index}`], 500);
      const item = JSON.parse(Buffer.from(encoded, "base64url").toString("utf8"));
      const line = {
        productId: safeText(item.p, 80),
        variantId: safeText(item.v, 80),
        quantity: Math.max(1, Math.floor(Number(item.q || 1))),
        priceCop: Math.max(1, Math.round(Number(item.a || 0))),
        amount: Math.max(1, Math.round(Number(item.a || 0))),
        name: safeText(item.n, 100) || "Producto CajaModa",
        fulfillmentCode: safeText(item.f, 10).toUpperCase(),
        selectedDeliveryMode: safeText(item.d, 20).toLowerCase()
      };
      if (line.productId && line.variantId) lines.push(line);
    } catch {
      // A malformed line is rejected by the final length check.
    }
  }
  return lines;
}

async function protectCheckout3Payment(response, operation) {
  try {
    await operation();
  } catch (error) {
    const diagnosticCode = `C3${Date.now().toString(36).slice(-7).toUpperCase()}${crypto.randomBytes(3).toString("hex").toUpperCase()}`;
    console.error(`[Checkout 3 ${diagnosticCode}]`, error);
    if (error instanceof CartItemUnavailableError) {
    ...
    sendJson(response, 422, {
      code: "CHECKOUT_3_PREPARATION_FAILED",
      message: safeText(error?.message, 300) || "No pudimos preparar el pago.",
      diagnosticCode
    });
  }
}

async function handlePrepareCheckout3Payment(request, response) {
  if (!stripe || !STRIPE_PUBLISHABLE_KEY) throw new Error("Stripe no está configurado.");
  const body = await readBody(request);
  const checkoutToken = safeText(body?.checkoutToken, 100);
  const items = Array.isArray(body?.cart?.items) ? body.cart.items.slice(0, 25) : [];
  if (!checkoutToken || !items.length) throw new Error("La bolsa no es válida.");
  ...
  const lines = verified.map(line => ({
    ...
    variantId: safeText(line?.price_data?.product_data?.metadata?.variantId, 80),
    quantity: Math.max(1, Math.floor(Number(line.quantity || 1))),
    priceCop: Math.round(Number(line?.price_data?.unit_amount || 0) / 100),
    name: safeText(line?.price_data?.product_data?.name, 180) || "Producto CajaModa",
    fulfillmentCode: safeText(line?.fulfillmentCode, 10).toUpperCase(),
    selectedDeliveryMode: safeText(line?.selectedDeliveryMode, 20).toLowerCase()
  }));
  const delivery = await checkoutDelivery(body, lines);
  const subtotalCop = lines.reduce((sum, line) => sum + line.priceCop * line.quantity, 0);
  const deliveryCop = Math.max(0, Math.round(Number(delivery.fee || 0)));
  const totalCop = subtotalCop + deliveryCop;
  const amountMinor = totalCop * 100;
  if (!Number.isSafeInteger(amountMinor) || amountMinor < 100) throw new Error("El total del pedido no es válido.");
  ...
  if (!customerEmail || !customerPhone) throw new Error("Completa el correo y el celular.");
  ...
  const intent = await stripe.paymentIntents.create({
    ...
    metadata: checkout3PaymentMetadata(lines, body, delivery)
  }, {
    idempotencyKey: `checkout3:${checkoutToken}:${safeText(body?.requestId, 120)}`
  });
  ...
  sendJson(response, 201, {
    ...
    currency: "COP",
    amountMinor,
    totals: { subtotalCop, deliveryCop, totalCop },
    delivery: { title: delivery.title, method: delivery.method }
  });
}

async function findCheckout3WixOrder(paymentIntentId) {
  const result = await wix.orders.searchOrders({
    filter: { "channelInfo.externalOrderId": paymentIntentId },
    cursorPaging: { limit: 5 }
  });
  return (result?.orders || [])[0] || null;
}

async function importCheckout3PaymentIntoWix(paymentIntent, lines) {
  const existing = await findCheckout3WixOrder(paymentIntent.id);
  if (existing) return { order: existing, created: false };
  ...
  const customerName = billing.name || paymentIntent.metadata.customerName || paymentIntent.shipping?.name;
  const name = splitCustomerName(customerName);
  const email = safeText(billing.email || paymentIntent.receipt_email || paymentIntent.metadata.customerEmail, 250);
  const phone = safeText(billing.phone || paymentIntent.shipping?.phone || paymentIntent.metadata.customerPhone, 80);
  const deliveryAddress = {
    country: "CO",
    city: safeText(paymentIntent.shipping?.address?.city || paymentIntent.metadata.deliveryCity, 100),
    subdivision: safeText(paymentIntent.shipping?.address?.state || paymentIntent.metadata.deliveryState, 100),
    postalCode: safeText(paymentIntent.shipping?.address?.postal_code || paymentIntent.metadata.deliveryPostalCode, 40),
    addressLine: safeText(paymentIntent.shipping?.address?.line1 || paymentIntent.metadata.deliveryAddress, 250)
  };
  const billingAddress = {
    country: "CO",
    city: safeText(billing?.address?.city, 100) || deliveryAddress.city,
    subdivision: safeText(billing?.address?.state, 100) || deliveryAddress.subdivision,
    postalCode: safeText(billing?.address?.postal_code, 40) || deliveryAddress.postalCode,
    addressLine: safeText(billing?.address?.line1, 250) || deliveryAddress.addressLine
  };
  const subtotalCop = lines.reduce((sum, line) => sum + line.priceCop * line.quantity, 0);
  const deliveryCop = Math.max(0, Math.round(Number(paymentIntent.metadata.deliveryFeeCop || 0)));
  const totalCop = Math.round(Number(paymentIntent.amount_received || paymentIntent.amount || 0) / 100);

  const imported = await wix.orders.importOrder({
    status: "APPROVED",
    paymentStatus: "PAID",
    fulfillmentStatus: "NOT_FULFILLED",
    channelInfo: { type: "OTHER_PLATFORM", externalOrderId: paymentIntent.id },
    currency: "COP",
    currencyConversionDetails: { originalCurrency: "COP", conversionRate: "1" },
    buyerInfo: { email },
    billingInfo: {
      contactDetails: { firstName: name.firstName, lastName: name.lastName, email, phone },
      address: billingAddress
    },
    shippingInfo: {
      title: safeText(paymentIntent.metadata.deliverySummary, 300) || "Entrega CajaModa",
      cost: { amount: String(deliveryCop) },
    ...
    lineItems: lines.map(line => ({
      productName: { original: line.name },
      descriptionLines: [{ name: { original: "Entrega" }, plainText: { original: selectedDeliveryLabel(line.selectedDeliveryMode) } }],
      quantity: line.quantity,
      price: { amount: String(line.priceCop) },
      itemType: { preset: "PHYSICAL" },
      physicalProperties: { shippable: true },
      catalogReference: { appId: WIX_STORES_APP_ID, catalogItemId: line.productId, options: { variantId: line.variantId } }
    })),
    priceSummary: {
      subtotal: { amount: String(subtotalCop) }, shipping: { amount: String(deliveryCop) },
      tax: { amount: "0" }, discount: { amount: "0" }, total: { amount: String(totalCop) }
    }
  });
  return { order: imported?.order || imported, created: true };
}

async function syncCheckout3Payment(paymentIntent) {
  const active = checkout3SyncLocks.get(paymentIntent.id);
  if (active) return active;
  const sync = (async () => {
    if (!wix) throw new Error("Wix no está configurado para recibir el pedido.");
    if (paymentIntent.status !== "succeeded" || paymentIntent.metadata?.source !== "cajamoda-checkout-3") return null;
    const latest = await stripe.paymentIntents.retrieve(paymentIntent.id, { expand: ["payment_method"] });
    if (latest.metadata?.checkout3WixSync === "complete") return findCheckout3WixOrder(latest.id);
    const lines = checkout3PaymentLines(latest);
    if (!lines.length || lines.length !== Number(latest.metadata.itemCount)) throw new Error("El pago no contiene un pedido verificable.");
    const imported = await importCheckout3PaymentIntoWix(latest, lines);
    if (imported.created) await decrementStripeInventory(lines);
    await stripe.paymentIntents.update(latest.id, { metadata: {
      ...latest.metadata,
      checkout3WixSync: "complete",
      checkout3WixOrderId: safeText(imported?.order?._id || imported?.order?.id, 80)
    } });
    return imported.order;
  })();
  checkout3SyncLocks.set(paymentIntent.id, sync);
  try { return await sync; }
  finally { checkout3SyncLocks.delete(paymentIntent.id); }
}

async function handleCheckout3Confirmation(_request, response, url) {
  if (!stripe) return sendError(response, 503, "Stripe no está configurado.");
  const paymentIntentId = safeText(url.searchParams.get("paymentIntent"), 100);
  if (!/^pi_[A-Za-z0-9]+$/.test(paymentIntentId)) return sendError(response, 400, "El pago no es válido.");
  const intent = await stripe.paymentIntents.retrieve(paymentIntentId, { expand: ["payment_method"] });
  if (intent.metadata?.source !== "cajamoda-checkout-3") return sendError(response, 404, "El pago no pertenece a este checkout.");
  if (intent.status !== "succeeded") return sendJson(response, 202, { ok: false, status: intent.status, message: "El pago todavía está en proceso." });
  const order = await syncCheckout3Payment(intent) || await findCheckout3WixOrder(intent.id);
  sendJson(response, 200, {
    ok: true,
    order: {
      paid: true,
      number: safeText(order?.number, 100) || `S-${intent.id.slice(-12).toUpperCase()}`,
      payment: "Pago con tarjeta confirmado",
      delivery: getConfirmationDelivery({ shippingInfo: { title: safeText(intent.metadata.deliverySummary, 300) } }),
  ...
  setImmediate(() => {
    void (async () => {
      ...
      if (event.type === "payment_intent.succeeded") {
        if (event.data.object?.metadata?.source === "cajamoda-checkout-3") {
          await syncCheckout3Payment(event.data.object);
          console.log(`[Stripe Checkout 3] Payment synchronized with Wix: ${event.data.object.id}`);
        } else {
  ...
  http.createServer(
    async (
      ...
      try {
        ...
        if(request.method === "POST" && url.pathname === "/api/checkout-3/payment/prepare"){
          await protectCheckout3Payment(response,() => handlePrepareCheckout3Payment(request,response));
          return;
        }

        if(request.method === "GET" && url.pathname === "/api/checkout-3/payment/confirmation"){
          await handleCheckout3Confirmation(request,response,url);
          return;
        }
